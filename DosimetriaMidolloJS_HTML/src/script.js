...
function computeDose() {
  ...
  const doseBlood = Sblood * AbloodEff * IA_MBq; // sangue
...
if (radioSel) {
  radioSel.addEventListener('change', e => {
    ...
    if ($('#kpiLambda')) $('#kpiLambda').value = lambda.toFixed(3);

    console.log(`📘 Isotopo: ${iso} → T½=${def.t12} d, τ=${def.td} d, λ=${lambda.toFixed(3)} d⁻¹`);
...
const activity_2Gy_GBq = total_Gy_per_GBq > 0 ? 2 / total_Gy_per_GBq : 0;
  ...
  console.log("→ Attività 2 Gy (GBq)", activity_2Gy_GBq);
  
 // 🔥 Salvo per UI e per il report
state.kpi.activity2Gy = activity_2Gy_GBq; 
...
<div class="kpi-card">
  τ<sub>Blood</sub><br><b>${fmtTauSmart(tauBlood)}</b> h
      ...
      <div class="kpi-card">
        Dose Midollo (AIFM)<br><b>${AIFM_Gy.toFixed(3)}</b> Gy
      </div>
      <div class="kpi-card">
        Dose Midollo (Traino)<br><b>${Traino_Gy.toFixed(3)}</b> Gy
      </div>

<div class="kpi-card">
  Dose Sangue (EANM 2008)<br>
  <b>${Sangue_Gy.toFixed(3)}</b> Gy<br>
  <small>Self: ${selfDose_Gy.toFixed(3)} Gy — Cross: ${crossDose_Gy.toFixed(3)} Gy</small>
</div>

      <div class="kpi-card dose2gy" style="background:${riskColor};color:#fff;">
        Attività per 2 Gy al Sangue<br><b>${act2Gy_GBq.toFixed(2)}</b> GBq
      </div>
    </div>
  `;
